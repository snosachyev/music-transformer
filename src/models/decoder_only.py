...
class DecoderOnlyMusicTransformer(nn.Module):
    ...
    def forward(self, dec_in, tgt_mask=None):
        """
        dec_in: (B, L, 3)
        tgt_mask: (L, L) boolean causal mask
        """
        x = self.embed(dec_in)  # (B, L, d_model)

        # если маску не подали — создаём
        if tgt_mask is None:
            L = dec_in.size(1)
            tgt_mask = torch.triu(
                torch.ones(L, L, dtype=torch.bool, device=dec_in.device),
                diagonal=1
            )

        # memory = x (decoder-only self-attention)
